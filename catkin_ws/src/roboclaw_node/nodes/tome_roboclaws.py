...
class Node:
    ...
    def check_vitals_back_motors(self, stat):
        try:
            status = roboclaw.ReadError(self.addressBackMotors)[1]
        except OSError as e:
        ...
        try:
            stat.add("Main Batt V:", float(roboclaw.ReadMainBatteryVoltage(self.addressBackMotors)[1] / 10))
            stat.add("Logic Batt V:", float(roboclaw.ReadLogicBatteryVoltage(self.addressBackMotors)[1] / 10))
            stat.add("Temp1 C:", float(roboclaw.ReadTemp(self.addressBackMotors)[1] / 10))
            stat.add("Temp2 C:", float(roboclaw.ReadTemp2(self.addressBackMotors)[1] / 10))
        except OSError as e:
            rospy.logwarn("Diagnostics OSError: %d", e.errno)
            rospy.logdebug(e)
        return stat
